...
word f_pc = [
    ...
    W_icode == IRET : W_valM;
    # Implement iaddq $8, %rsi
    f_icode == IIADDQ && W_icode == IJXX : M_PC;
    f_icode == IIADDQ && W_icode == IIADDQ && D_valC == 8 && W_dstM != RNONE : W_dstM;
...
word d_dstM = [
    ...
    D_icode == IIADDQ && D_valC == 7 : 10;
    D_icode == IIADDQ && D_valC == 8 && m_valM < 300 : M_PC;
    1 : RNONE;  # Don't write any register
];
...
word d_valA = [
    ...
    D_icode == IIADDQ && D_valC == 7 : W_valE;
    D_icode == IIADDQ && D_valC == 8 : m_valM;
...
word aluA = [
    ...
    E_icode in { IIADDQ } && E_valC in { 7 } : 8;
    E_icode in { IIADDQ } && E_valC == -2022 : 0;
    E_icode in { IIADDQ } && E_valC == 1 && m_valM > 0 && m_valM < 300 : 1;
    E_icode in { IIADDQ } && E_valC == 1 : 0;
...
word e_valA = [
    E_icode == IIADDQ && E_valC == 7 && M_icode == IJXX : E_valB;
    E_icode == IIADDQ && E_valC == 7 : E_valA;
    E_icode == IIADDQ && E_valC == 1 : m_valM;
...
bool mem_read = M_icode in { IMRMOVQ, IPOPQ, IRET } || (M_icode == IIADDQ && E_valC == 1);

## Set write control signal
bool mem_write = M_icode in { IRMMOVQ, IPUSHQ, ICALL } || (M_icode == IIADDQ && E_valC == 8 && M_valA < 300);
