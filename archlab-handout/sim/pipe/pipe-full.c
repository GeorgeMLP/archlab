...
long long gen_f_pc()
{
    return ((((ex_mem_curr->icode) == (I_JMP)) & (!
        ...
        (mem_wb_curr->icode) == (I_RET)) ? (mem_wb_curr->valm) : ((
          (if_id_next->icode) == (I_IADDQ)) & ((mem_wb_curr->icode) == 
          (I_JMP))) ? (ex_mem_curr->stage_pc) : (((((if_id_next->icode) == 
              (I_IADDQ)) & ((mem_wb_curr->icode) == (I_IADDQ))) & (
            (if_id_curr->valc) == 8)) & ((mem_wb_curr->destm) != (REG_NONE))
        ) ? (mem_wb_curr->destm) : (pc_curr->pc));
}
...
{
    return ((imem_error) ? (F_NONE) : ((if_id_next->icode) == (I_IADDQ)) ? 6
       : (imem_ifun));
}
...
long long gen_d_dstM()
{
    return (((if_id_curr->icode) == (I_MRMOVQ) || (if_id_curr->icode) == 
        (I_POPQ)) ? (if_id_curr->ra) : (((if_id_curr->icode) == (I_IADDQ))
         & ((if_id_curr->valc) == 7)) ? 10 : ((((if_id_curr->icode) == 
            (I_IADDQ)) & ((if_id_curr->valc) == 8)) & ((mem_wb_next->valm)
           < 300)) ? (ex_mem_curr->stage_pc) : (REG_NONE));
}

long long gen_d_valA()
{
    return (((if_id_curr->icode) == (I_CALL) || (if_id_curr->icode) == 
        (I_JMP)) ? (if_id_curr->valp) : (((if_id_curr->icode) == (I_IADDQ))
         & ((if_id_curr->valc) == 7)) ? (mem_wb_curr->vale) : ((
          (if_id_curr->icode) == (I_IADDQ)) & ((if_id_curr->valc) == 8)) ? 
      (mem_wb_next->valm) : ((id_ex_next->srca) == (ex_mem_next->deste)) ? 
...
long long gen_aluA()
{
    return (((id_ex_curr->icode) == (I_RRMOVQ) || (id_ex_curr->icode) == 
        ...
        (I_MRMOVQ)) ? (id_ex_curr->valc) : (((id_ex_curr->icode) == 
          (I_IADDQ)) & ((id_ex_curr->valc) == 7)) ? 8 : (((id_ex_curr->icode)
           == (I_IADDQ)) & ((id_ex_curr->valc) == -2022)) ? 0 : ((((
              (id_ex_curr->icode) == (I_IADDQ)) & ((id_ex_curr->valc) == 1))
           & ((mem_wb_next->valm) > 0)) & ((mem_wb_next->valm) < 300)) ? 1 : ((
          (id_ex_curr->icode) == (I_IADDQ)) & ((id_ex_curr->valc) == 1)) ? 0
...
{
    return (((((id_ex_curr->icode) == (I_IADDQ)) & ((id_ex_curr->valc) == 7)
          ) & ((ex_mem_curr->icode) == (I_JMP))) ? (id_ex_curr->valb) : ((
          (id_ex_curr->icode) == (I_IADDQ)) & ((id_ex_curr->valc) == 7)) ? 
      (id_ex_curr->vala) : (((id_ex_curr->icode) == (I_IADDQ)) & (
          (id_ex_curr->valc) == 1)) ? (mem_wb_next->valm) : 
      (id_ex_curr->vala));
}
...
long long gen_mem_addr()
{
    return (((ex_mem_curr->icode) == (I_RMMOVQ) || (ex_mem_curr->icode) == 
        ...
        ) ? (ex_mem_curr->vala) : (((ex_mem_curr->icode) == (I_IADDQ)) & (
          (id_ex_curr->valc) == 1)) ? (ex_mem_curr->vala) : (((
            (ex_mem_curr->icode) == (I_IADDQ)) & ((id_ex_curr->valc) == 8))
         & ((ex_mem_curr->vala) < 300)) ? (id_ex_curr->valb) : 0);
}
...
long long gen_mem_read()
{
    return (((ex_mem_curr->icode) == (I_MRMOVQ) || (ex_mem_curr->icode) == 
        (I_POPQ) || (ex_mem_curr->icode) == (I_RET)) | ((
          (ex_mem_curr->icode) == (I_IADDQ)) & ((id_ex_curr->valc) == 1)));
}
